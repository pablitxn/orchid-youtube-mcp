...
def _create_test_app(monkeypatch, manager: _FakeManager | None):
    monkeypatch.setattr("src.api.main.get_settings", lambda: _make_settings())
    monkeypatch.setattr("src.api.main.init_services", AsyncMock())
    monkeypatch.setattr("src.api.main.shutdown_services", AsyncMock())
    monkeypatch.setattr(
        "src.api.openapi.routes.health.get_runtime_manager",
        lambda: manager,
    )
    return create_app()
